...
const TrendingApps = ({ fetchAppsPromise }) => {
  const trendingApps = use(fetchAppsPromise);
        ...
        <div key={app.id} className="rounded-xl bg-white">
          ...
          <div className="flex justify-between items-center">
            <div className="p-2 bg-[#F1F5E8] flex items-center gap-2 rounded-md">
                <img className="w-4 h-4" src={DownloadIcone} alt="Download Icon" />
                <p className="text-[#00D390] text-md font-semibold">{app.downloads}</p>
            </div>
            <div className="p-2 bg-[#FFF0E1] flex items-center gap-2 rounded-md ">
                <img className="w-4 h-4"  src={StarIcone} alt="Star Icon" />
                <p className="text-[#FF8811] text-md font-semibold">{app.ratingAvg}</p>
            </div>
          </div>
        </div>
      ))}
